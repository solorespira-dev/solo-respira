...
$queryUsuario = $conn->prepare("SELECT id FROM usuarios WHERE email = ?");
...
$queryDonaciones = $conn->prepare("SELECT SUM(monto) AS total FROM donaciones WHERE usuario_id = ?");
...
// Próximo evento
$queryEvento = $conn->query("SELECT nombre, fecha FROM eventos WHERE fecha >= CURDATE() ORDER BY fecha ASC LIMIT 1");
$evento = $queryEvento->fetch_assoc();

// Últimos mensajes recibidos
$queryMensajes = $conn->prepare("SELECT m.id, u.nombre AS remitente, m.mensaje, m.fecha FROM mensajes m JOIN usuarios u ON m.remitente_id = u.id WHERE m.destinatario_id = ? ORDER BY m.fecha DESC LIMIT 3");
$queryMensajes->bind_param("i", $userId);
...
<div class="sidebar">
    <img src="images/LOGO SOLO RESPIRA.png" alt="Logo Fundación Solo Respira">
